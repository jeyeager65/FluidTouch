...
bool PowerManager::supportsDimmedState()
{
...
void PowerManager::init(DisplayDriver *driver)
{
  ...
  if (!supportsDimmedState())
  {
    Serial.println("Dimmed state: NOT SUPPORTED on this hardware (switch-only backlight)");
  }
...
void PowerManager::update(int machine_state)
{
  ...
  case FULL_BRIGHTNESS:
    // Check if we should dim (only if dimming is enabled and hardware supports it)
    if (supportsDimmedState() && dim_timeout_sec > 0 && idle_sec >= dim_timeout_sec)
    {
      enterDimmed();
    }
    // Hardware without analog dimming skips straight to screen off
    else if (!supportsDimmedState() && sleep_timeout_sec > 0 && idle_sec >= sleep_timeout_sec)
    {
      enterScreenOff();
    }
    break;
  ...
  case DIMMED:
    // If hardware does not support dimming, recover to a valid state
    if (!supportsDimmedState())
    {
      enterFullBrightness();
      break;
...
void PowerManager::loadSettings()
{
  ...
  if (dim_timeout_sec > 0 && dim_timeout_sec < 10)
    dim_timeout_sec = 10;
  if (dim_timeout_sec > 600)
    dim_timeout_sec = 600;
  if (sleep_timeout_sec > 0 && sleep_timeout_sec < 10)
    sleep_timeout_sec = 10;
  if (sleep_timeout_sec > 3600)
    sleep_timeout_sec = 3600;
  ...
  if (deep_sleep_timeout_sec > 0 && deep_sleep_timeout_sec < 300)
    deep_sleep_timeout_sec = 300;
  if (deep_sleep_timeout_sec > 7200)
    deep_sleep_timeout_sec = 7200; // Max 2 hours
  if (normal_brightness > 100)
    normal_brightness = 100; // Validate percentage range
  if (dim_brightness > 100)
    dim_brightness = 25; // Validate percentage range
}
...
void PowerManager::enterDimmed()
{
  if (!supportsDimmedState())
  {
    Serial.println("PowerManager: DIMMED requested, but this hardware only supports on/off backlight");
    return;
  }
