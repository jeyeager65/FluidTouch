...
{
  {
...
#if defined(HARDWARE_WAVESHARE_7)
    // Waveshare ESP32-S3-Touch-LCD-7 (RGB565 over 16-bit RGB bus)
    // Reference: docs.waveshare.com/ESP32-S3-Touch-LCD-7 (Interface Description)
    ...
    cfg.freq_write = 16000000; // Waveshare reference timing: 16MHz
...
#if defined(HARDWARE_ADVANCE) || defined(HARDWARE_WAVESHARE_7)
...
#if defined(HARDWARE_WAVESHARE_7)
    // Waveshare: touch shares I2C bus on GPIO8/9
    cfg.i2c_port = 0;
    cfg.i2c_addr = 0x5D;
    cfg.pin_sda = TOUCH_SDA; // GPIO 8
    cfg.pin_scl = TOUCH_SCL; // GPIO 9
    cfg.pin_rst = -1;        // TP_RST routed via CH422G expander
    cfg.pin_int = TOUCH_INT; // GPIO 4
    cfg.freq = 400000;
#elif defined(HARDWARE_ADVANCE)
...
#elif defined(BACKLIGHT_EXPANDER)
  // Waveshare: backlight is switched via CH422G I/O expander.
  Serial.println("Initializing Waveshare hardware (CH422G backlight/IO expander)...");
  if (!WaveshareIOExpander::init())
  {
    Serial.println("ERROR: Failed to initialize Waveshare CH422G expander");
    return false;
  }
...
#else
#error "No backlight type defined! Use -DBACKLIGHT_PWM, -DBACKLIGHT_I2C, or -DBACKLIGHT_EXPANDER"
#endif
...
#ifdef BACKLIGHT_EXPANDER
  WaveshareIOExpander::setBacklight(true);
  Serial.println("Backlight ON (CH422G)");
#endif
...
#elif defined(BACKLIGHT_EXPANDER)
  // Waveshare path: CH422G provides switched backlight control only, not PWM dimming.
  WaveshareIOExpander::setBacklight(hw_value > 0);
#elif defined(BACKLIGHT_I2C)
  // Advance: I2C backlight controller (STC8H1K28 at address 0x30)
  uint8_t i2c_value;
#ifdef HARDWARE_ADVANCE_V12
...
#elif defined(BACKLIGHT_EXPANDER)
  WaveshareIOExpander::setBacklight(false);
  Serial.println("Backlight OFF (CH422G)");
#elif defined(BACKLIGHT_I2C)
  // Advance: I2C backlight controller (STC8H1K28 at address 0x30)
#ifdef HARDWARE_ADVANCE_V12
