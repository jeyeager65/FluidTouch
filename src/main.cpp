...
void loop()
{
  ...
  if (currentMillis - lastUIUpdate >= 250)
  {
    ...
    else
    {
      ...
      // Keep setup/selection screens responsive when no machine is selected.
      // If we allow inactivity power-save here, the UI can go black and appear frozen
      // before the user has configured connectivity.
      ...
      {
        // Selected machine exists but currently disconnected -> normal power policy applies.
        PowerManager::update(STATE_DISCONNECTED);
      }
    }
  }
