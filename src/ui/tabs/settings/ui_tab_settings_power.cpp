...
static constexpr bool kSupportsDimControl = false;
#else
static constexpr bool kSupportsDimControl = true;
#endif
...
void UITabSettingsPower::create(lv_obj_t *tab)
{
  ...
  if (!kSupportsDimControl)
  {
    lv_label_set_text(dim_label, "Dim After:");
    lv_dropdown_set_options(dim_timeout_dropdown, "Not supported");
    lv_dropdown_set_selected(dim_timeout_dropdown, 0);
  ...
  if (sleep_sec == 0)
    sleep_idx = 0; // Disabled
  else if (sleep_sec == 60)
    sleep_idx = 1;
  else if (sleep_sec == 120)
    sleep_idx = 2;
  else if (sleep_sec == 300)
    sleep_idx = 3;
  else if (sleep_sec == 600)
    sleep_idx = 4;
  else if (sleep_sec == 900)
    sleep_idx = 5;
  else if (sleep_sec == 1800)
    sleep_idx = 6;
  else if (sleep_sec == 3600)
    sleep_idx = 7;
  ...
  if (normal_bright <= 25)
    normal_idx = 0; // 25%
  else if (normal_bright <= 50)
    normal_idx = 1; // 50%
  else if (normal_bright <= 75)
    normal_idx = 2; // 75%
  else
    normal_idx = 3; // 100%
  ...
  if (dim_bright <= 5)
    bright_idx = 0; // 5%
  else if (dim_bright <= 10)
    bright_idx = 1; // 10%
  else if (dim_bright <= 25)
    bright_idx = 2; // 25%
  else
    bright_idx = 3; // 50%
  ...
  {
    lv_label_set_text(brightness_label, "Dim Level:");
    lv_dropdown_set_options(dim_brightness_dropdown, "Not supported");
    lv_dropdown_set_selected(dim_brightness_dropdown, 0);
  ...
  if (deep_sleep_sec == 0)
    deep_idx = 0; // Disabled
  else if (deep_sleep_sec <= 300)
    deep_idx = 1; // 5 min
  else if (deep_sleep_sec <= 600)
    deep_idx = 2; // 10 min
  else if (deep_sleep_sec <= 900)
    deep_idx = 3; // 15 min
  else if (deep_sleep_sec <= 1800)
    deep_idx = 4; // 30 min
  else if (deep_sleep_sec <= 3600)
    deep_idx = 5; // 60 min
  else
    deep_idx = 6; // 90 min
  ...
  {
    lv_label_set_text(note_label, "Note: Power management is active during IDLE/OFFLINE. Waveshare backlight is switch-only, so dimming is not available.");
  }
...
static void btn_save_power_event_handler(lv_event_t *e)
{
  ...
  if (code == LV_EVENT_CLICKED)
  {
    ...
    // Get dim timeout from dropdown (if supported by current hardware)
    if (kSupportsDimControl)
    {
    ...
    // Get dim brightness from dropdown (if supported by current hardware)
    if (kSupportsDimControl)
    {
...
static void btn_reset_event_handler(lv_event_t *e)
{
  ...
  if (code == LV_EVENT_CLICKED)
  {
    ...
    lv_obj_add_state(power_mgmt_switch, LV_STATE_CHECKED);
    if (kSupportsDimControl)
      lv_dropdown_set_selected(dim_timeout_dropdown, 2);     // 30 sec
    ...
    // Keep dim controls disabled on switch-only hardware
    if (!kSupportsDimControl)
    {
      lv_obj_add_state(dim_timeout_dropdown, LV_STATE_DISABLED);
      lv_obj_add_state(dim_brightness_dropdown, LV_STATE_DISABLED);
...
static void power_mgmt_switch_event_handler(lv_event_t *e)
{
  ...
  if (code == LV_EVENT_VALUE_CHANGED)
  {
    ...
    // Keep dim controls disabled on switch-only hardware, regardless of master toggle
    if (!kSupportsDimControl)
    {
      lv_obj_add_state(dim_timeout_dropdown, LV_STATE_DISABLED);
      lv_obj_add_state(dim_brightness_dropdown, LV_STATE_DISABLED);
