...
bool SettingsManager::exportSettings(const char *filepath)
{
  ...
  {
    if (!machine_configs[i].is_configured)
      continue;
...
bool SettingsManager::importSettings(const char *filepath)
{
  ...
  if (!system.isNull())
  {
    ...
    if (!power.isNull())
    {
      ...
      // Waveshare backlight is switch-only (CH422G), so dim-specific values are ignored.
      if (!power["dim_timeout"].isNull())
      {
        Serial.printf("[SettingsManager] INFO - Ignoring imported dim_timeout=%lu on Waveshare (switch-only backlight)\n", (uint32_t)(power["dim_timeout"] | 0));
      }
      if (!power["dim_brightness"].isNull())
      {
        Serial.printf("[SettingsManager] INFO - Ignoring imported dim_brightness=%u on Waveshare (switch-only backlight)\n", (uint8_t)(power["dim_brightness"] | 0));
      }
#else
...
#ifdef HARDWARE_WAVESHARE_7
  Serial.println("[SettingsManager] INFO: Dim-specific power settings are ignored on Waveshare (switch-only backlight).");
#endif
  return true;
}
