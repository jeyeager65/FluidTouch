...
bool UploadManager::init()
{
  ...
  Serial.println("[UploadManager] Waveshare: SPI mode with CH422G SD_CS (MOSI=11, MISO=13, CLK=12, CS=EXIO4)");

  if (!WaveshareIOExpander::init())
  {
    Serial.println("[UploadManager] Failed to initialize CH422G expander");
    return false;
  }

  // SD chip select is controlled by CH422G, not by an ESP32 GPIO.
  WaveshareIOExpander::selectSDCard(true);
  SPI.setHwCs(false);
  SPI.begin(SD_CLK, SD_MISO, SD_MOSI, -1);

  if (!SD.begin(SD_CS, SPI, 4000000, "/sd", 5, false))
  {
    WaveshareIOExpander::selectSDCard(false);
                               ...
                               CompleteCallback onComplete)
{
  if (_uploading)
  {
    ...
    if (onComplete)
      onComplete(false, "Upload already in progress");
    return false;
  }
  ...
  if (!file)
  {
    ...
    _uploading = false;
    if (onComplete)
      onComplete(false, "Failed to open local file");
    return false;
  }
  ...
  { // 10MB limit
    ...
    _uploading = false;
    if (onComplete)
      onComplete(false, "File too large (max 10MB)");
    return false;
  }
  ...
  if (!buffer)
  {
    ...
    _uploading = false;
    if (onComplete)
      onComplete(false, "Out of memory");
    return false;
  }
  ...
  if (machineIP.isEmpty())
  {
    ...
    _uploading = false;
    if (onComplete)
      onComplete(false, "FluidNC not connected");
    return false;
  }
  ...
  if (machineIP.indexOf('.') == -1)
  {
    ...
    if (!WiFi.hostByName(machineIP.c_str(), serverIP))
    {
      ...
      _uploading = false;
      if (onComplete)
        onComplete(false, "Failed to resolve hostname");
      return false;
    }
  ...
  if (!client.connect(machineIP.c_str(), 80))
  {
    ...
    _uploading = false;
    if (onComplete)
      onComplete(false, "Connection failed");
    return false;
  }
  ...
  if (success)
  {
    ...
    if (success)
    {
      ...
      if (response.indexOf("200") >= 0)
        httpCode = 200;
      else if (response.indexOf("201") >= 0)
        httpCode = 201;
      else
      {
